...
with DAG(
    ...
    extract_user_data = PostgresToGCSOperator(
        task_id='extract_user_data',
        sql="""SELECT email, first_name, last_name FROM public.event_enrolment_eventuser""",
        bucket=BQ_BUCKET,
        filename=f'user_data_{date_suffix}.csv',
        export_format='csv',
        postgres_conn_id=PG_CON_ID,
        gcp_conn_id=BQ_CON_ID
    )

    extract_score_data = PostgresToGCSOperator(
        task_id='extract_score_data',
        sql="""
            SELECT student_id, score, assessment_id, passed FROM public.assessments_score""",
        bucket=BQ_BUCKET,
        filename=f'score_data_{date_suffix}.csv',
        export_format='csv',
        postgres_conn_id=LLM_CON_ID,
        gcp_conn_id=BQ_CON_ID
    )

    def transform_and_merge_data(**kwargs):
        date_suffix = datetime.now().strftime("%Y%m%d")
        gcs_hook = GCSHook(gcp_conn_id=BQ_CON_ID)

        user_data_blob = f'user_data_{date_suffix}.csv'
        gcs_hook.download(
            bucket_name=BQ_BUCKET,
            object_name=user_data_blob,
            filename=f'/tmp/{user_data_blob}'
        )

        score_data_blob = f'score_data_{date_suffix}.csv'
        gcs_hook.download(
            bucket_name=BQ_BUCKET,
            object_name=score_data_blob,
            filename=f'/tmp/{score_data_blob}'
        )

        user_data = pd.read_csv(f'/tmp/{user_data_blob}')
        score_data = pd.read_csv(f'/tmp/{score_data_blob}')

        user_data.rename(columns={'email': 'student_id'}, inplace=True)
        merged_data = pd.merge(user_data, score_data, on='student_id', how='inner')

        merged_data_path = f'/tmp/merged_data_{date_suffix}.csv'
        merged_data.to_csv(merged_data_path, index=False)
        
        merged_data_blob = f'merged_data_{date_suffix}.csv'
        gcs_hook.upload(
            bucket_name=BQ_BUCKET,
            object_name=merged_data_blob,
            filename=merged_data_path
        )
        
        return merged_data_blob
    ...
    load_to_bigquery = GCSToBigQueryOperator(
        ...
        bucket=BQ_BUCKET,
        source_objects=[f'merged_data_{date_suffix}.csv'],
    ...
    extract_user_data >> extract_score_data >> transform_and_merge >> load_to_bigquery
